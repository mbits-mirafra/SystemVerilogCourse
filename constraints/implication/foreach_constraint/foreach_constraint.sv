...
//and set each element to the foreach_constraint of its particular index
...
initial begin
  ...
  //handle name is foreach_cons
  foreach_constraint foreach_cons;

  //memory allocation to the handle
  foreach_cons = new();
  $display("-------------------------------");
  void'(foreach_cons.randomize());
  $display("\t s_array1=%0p" , foreach_cons.s_array1);
  $display("\t s_array2=%0p" , foreach_cons.s_array2);
