...
module AND_Gate;
  ...
  initial begin
    ...
    // Apply test cases every delay time units
    #10;
    A<=0;
    B<=1;
    ...
    // Wait for 30 time units before terminating the simulation
    #30;
    // End the simulation
    $finish;
  end
  ...
  assert property (p) 
    $display("time=%0t,A=%0b and B=%0b, assertion success\n",$time,A,B);
  else 
    $display("time=%0t, A=%0b and B=%0b,assertion failure\n", $time,A,B);
